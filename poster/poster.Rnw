...
\title{\Huge Open Computational Ecosystems for Social Science}
\date{}
\author{{Markus Kainu, Joona Lehtomäki, Juuso Parkkinen, Juha Yrjölä, Måns Magnusson, Mikko Tolonen, Niko Ilomäki, Leo Lahti}\\ \Large Contact: http://ropengov.github.io}
...
\node [myboxwhite] (intro){%
    ...
    \section*{Open data creates opportunities for social sciences}
...
leaving more time to solve the specific research problems. Successful 
...
languages.
% The resulting communities, such as Bioconductor
...
\item \textbf{Open source}: We use GitHub for shared version control.
Openly licensed contributions guarantee that the tools
  are freely available for the international scientific community.
...
\item \textbf{Reproducible documentation}:
% High-quality documentation is critical
%   for package usability. 
  We provide online high-quality tutorials with fully
  reproducible documentation on how to access and analyse specific
  data sources, and to report the statistical results.

\item \textbf{Transparent research}: The data analysis steps from
raw data to the final results
are published in full detail.  To exemplify this, we publish reproducible case
  studies based on open data and algorithms in the rOpenGov blog.
% The programmatic approach makes it possible to
...
\item \textbf{Standardization}: A community-driven joint development 
approach helps to pool scarce
  research resources, develop common standards and ensure compatibilty for data
  analysis.
%   Joint development ensures that the applicability of the
...
\node [myboxwhite, right of=ecosystem, node distance=21cm] (ropengov){%
\begin{minipage}{0.23\textwidth}

\section*{rOpenGov - Ecosystem for computational social sciences and digital humanities}
...
fully automated and transparent fashion}.
...
\node [mybox, right of=ropengov, node distance=21cm, yshift=0cm] (packages){%
    \begin{minipage}{0.23\textwidth}
  ...
  \item GIS tools (OpenStreetMap, WFS, Finland)
...
\node [myboxwhite, below of=packages, node distance=35cm, yshift=0cm] (references){%
    \begin{minipage}{0.23\textwidth}

    We are thankful for a number of developers for supporting this
    community. For a full list, see http://ropengov.github.io.
...
%\begin{minipage}{0.5\textwidth}
    ...
    % Place nodes
    \node [greenbox, below of=example] (init) {Raw Data 1};
