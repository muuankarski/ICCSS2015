...
ESIMERKKIKUVA NÄISTÄ PAKETEISTA, OLIS KOVA.]
...
\node[fancytitle, right=10pt] at (box.north west) {Open Government Data};
...
\node[fancytitle] at (box.north) {API packages};
\end{tikzpicture}
...
\node[fancytitle] at (box.north) {Digital humanities};
\end{tikzpicture}
%
...
\node[fancytitle] at (box.north) {Parliamentary monitoring};
\end{tikzpicture}
%
...
\node [mybox] (box) {%
    ...
    \begin{itemize}
      ...
      \item usbroadband Data for the U.S. National Broadband Map 
